...
1. Exclude the dependency from the
   `spring-core`
   module \(as it is the only module that explicitly depends on
   `commons-logging`
   \)
2. Depend on a special
   `commons-logging`
   dependency that replaces the library with an empty jar \(more details can be found in the
